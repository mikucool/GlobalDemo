...
fun InterstitialAdPlatformTestScreen(
    ...
    viewModel: InterstitialAdTestViewModel = viewModel()
) {
    val context = LocalContext.current
    val state = viewModel.uiState.collectAsState()
    if (state.value.interstitialAdState == TestInterstitialAdState.LOADING) {
            ...
            Row(
                ...
                Button(onClick = {
                    val controller =
                        biddingAdControllers.find { it.adConfiguration.adPlatform == state.value.adPlatform }
                    if (controller != null) {
                        viewModel.displayInterstitialAd(context as Activity, controller)
                    }
                }) {
...
@Composable
fun InterstitialAdPlatformTestScreenPreview() {
